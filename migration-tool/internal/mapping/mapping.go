...
type Column struct {
	Name string
	// Type is the source column's MySQL type, recorded so the file can be
	// reviewed — and consumed — without reconnecting to the old database.
	Type string
	// Charset is the source column's character set, recorded only when it is
	// not Unicode, because that is when it changes what the transform has to
	// do.
	Charset string
	Rule    Rule
}
...
// ReadColumns returns, per legacy table, the columns the migration actually
// reads: every rule that copies or derives a value. Columns the plan skips are
// absent from the result, because a database that dropped one migrates fine and
// refusing to run over it would be a false alarm.
func ReadColumns() map[string][]string {
	...
	for table, tp := range Plan() {
		...
		var columns []string
		for name, rule := range tp.Columns {
			if rule.Action == ActionMap || rule.Action == ActionDerive {
...
func columnNodes(c Column) []*yaml.Node {
	body := newMapping()
	if c.Type != "" {
		addString(body, "type", c.Type, "")
	}
	if c.Charset != "" {
		addString(body, "charset", c.Charset, "")
	}
