...
- A difference is not automatically a failure. Only a missing required table, or a missing column a transformer actually reads, stops a run: the mapping skips 36 baseline columns outright, and an install that dropped one years ago migrates perfectly.
