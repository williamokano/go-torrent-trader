		...
		if deps != nil && deps.AuthService != nil {
			...
			if deps.ForumService != nil {
				...
				r.Route("/forums", func(r chi.Router) {
					...
					r.Post("/posts/{id}/restore", forums.HandleRestorePost)
					r.Get("/posts/{id}/edits", forums.HandleListPostEdits)
				...
				r.Group(func(r chi.Router) {
					...
					if deps.AdminService != nil {
						...
						if deps.HnRService != nil {
							...
							r.Get("/hnr/exempt-rules", hnr.HandleListExemptRules)
							r.Post("/hnr/exempt-rules", hnr.HandleCreateExemptRule)
							r.Put("/hnr/exempt-rules/{id}", hnr.HandleUpdateExemptRule)
							r.Delete("/hnr/exempt-rules/{id}", hnr.HandleDeleteExemptRule)
