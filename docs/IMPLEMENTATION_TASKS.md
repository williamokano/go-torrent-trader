...
#### BE-10: External Notification Connectors [L] [DONE — design in `docs/NOTIFICATION_CONNECTORS.md`, plan in `docs/plans/BE-10.md`; BE-10.5 remains future work]
**As a** tracker operator
...
- **A feed's filters are not an access control** — every member may open any feed and sees every enabled feed's name. Documented in `docs/NOTIFICATION_CONNECTORS.md` and in the admin form, because a dropdown of named feeds invites an operator to create "staff-only" and assume it is one. Per-feed group gating is BE-10.8 (not built).
...
#### BE-10.8: Per-feed access control [S] [PLANNED]
**As a** tracker operator
**I want** to restrict a live feed to a user class
**So that** "staff releases" is a feed staff can see and members cannot

**Context:** surfaced by the BE-10.7 review. Feeds today are a presentation choice: any authenticated member may open any slug, and `GET /api/v1/announce-feeds` lists every enabled feed's name to everyone. The multi-feed UI invites an operator to read a feed as a permission boundary, which it is not. Needs a group/class column on the instance, enforcement in `HandleStream` (which deliberately does not consult the connector list today — that decision would have to be revisited), and filtering in `PublicFeeds`.
