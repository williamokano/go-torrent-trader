...
func (m *mockForumRepo) Create(_ context.Context, forum *model.Forum) error {
	...
	if m.forumByID == nil {
		m.forumByID = make(map[int64]*model.Forum)
	}
...
func (m *mockForumRepo) RecalculateCounts(_ context.Context, forumID int64) error {
	m.recalculated = append(m.recalculated, forumID)
	return nil
}
...
func (m *mockForumTopicRepo) UpdateLastPost(_ context.Context, _ int64, _ int64, _ time.Time) error {
	return nil
}
func (m *mockForumTopicRepo) RecalculateLastPost(_ context.Context, _ int64) error { return nil }
func (m *mockForumTopicRepo) SetLocked(_ context.Context, id int64, locked bool) error {
	if m.lockedCalls == nil {
		m.lockedCalls = make(map[int64]bool)
	}
	m.lockedCalls[id] = locked
	return nil
}
func (m *mockForumTopicRepo) SetPinned(_ context.Context, id int64, pinned bool) error {
	if m.pinnedCalls == nil {
		m.pinnedCalls = make(map[int64]bool)
	}
	m.pinnedCalls[id] = pinned
	return nil
}
func (m *mockForumTopicRepo) UpdateTitle(_ context.Context, id int64, title string) error {
	if m.titleCalls == nil {
		m.titleCalls = make(map[int64]string)
	}
	m.titleCalls[id] = title
	return nil
}
func (m *mockForumTopicRepo) UpdateForumID(_ context.Context, id int64, forumID int64) error {
	if m.forumIDCalls == nil {
		m.forumIDCalls = make(map[int64]int64)
	}
	m.forumIDCalls[id] = forumID
	return nil
}
func (m *mockForumTopicRepo) Delete(_ context.Context, id int64) error {
...
func (m *mockForumPostRepo) Search(_ context.Context, _ string, _ *int64, _ int, _, _ int) ([]model.ForumSearchResult, int64, error) {
	return m.searchResults, m.searchTotal, m.searchErr
}
func (m *mockForumPostRepo) GetFirstPostIDByTopic(_ context.Context, _ int64) (int64, error) {
	return m.firstPostID, nil
}
func (m *mockForumPostRepo) SoftDelete(_ context.Context, id int64, deletedBy int64) error {
	m.softDeleted = id
	m.softDeletedBy = deletedBy
	return nil
}
func (m *mockForumPostRepo) Restore(_ context.Context, id int64) error { m.restored = id; return nil }
func (m *mockForumPostRepo) CreateEdit(_ context.Context, edit *model.ForumPostEdit) error {
	m.createdEdit = edit
	edit.ID = 1
	edit.CreatedAt = time.Now()
	return nil
}
func (m *mockForumPostRepo) ListEdits(_ context.Context, _ int64) ([]model.ForumPostEdit, error) {
	return m.edits, nil
}
...
func TestForumService_GetForum_AccessDenied(t *testing.T) {
	...
	if _, err := svc.GetForum(context.Background(), 1, model.Permissions{Level: 5}); !errors.Is(err, ErrForumAccessDenied) {
		t.Errorf("expected ErrForumAccessDenied, got %v", err)
	}
	if f, err := svc.GetForum(context.Background(), 1, model.Permissions{Level: 100}); err != nil {
		t.Fatalf("unexpected: %v", err)
	} else if f.Name != "VIP" {
		t.Errorf("expected VIP")
	}
}
...
func TestForumService_GetForum_NotFound(t *testing.T) {
	...
	if _, err := svc.GetForum(context.Background(), 999, model.Permissions{Level: 100}); !errors.Is(err, ErrForumNotFound) {
...
func TestForumService_CreateTopic_EmptyTitle(t *testing.T) {
	...
	if _, _, err := svc.CreateTopic(context.Background(), 1, 1, model.Permissions{}, "", "body"); !errors.Is(err, ErrInvalidTopic) {
		t.Errorf("expected ErrInvalidTopic")
	}
}

func TestForumService_CreateTopic_EmptyBody(t *testing.T) {
	svc := NewForumService(nil, nil, nil, nil, nil, nil, nil, nil)
	if _, _, err := svc.CreateTopic(context.Background(), 1, 1, model.Permissions{}, "Title", ""); !errors.Is(err, ErrInvalidPost) {
		t.Errorf("expected ErrInvalidPost")
	}
}
...
func TestForumService_CreateTopic_UserCanForumFalse(t *testing.T) {
	...
	if _, _, err := svc.CreateTopic(context.Background(), 1, 1, model.Permissions{Level: 5}, "Title", "Body"); !errors.Is(err, ErrForumAccessDenied) {
		t.Errorf("expected ErrForumAccessDenied")
	}
}
...
func TestForumService_CreatePost_TopicLocked(t *testing.T) {
	...
	if _, err := svc.CreatePost(context.Background(), 1, 1, model.Permissions{Level: 5}, "Reply", nil); !errors.Is(err, ErrTopicLocked) {
		t.Errorf("expected ErrTopicLocked")
	}
}
...
func TestForumService_CreatePost_TopicLocked_StaffAllowed(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("expected staff to post in locked topic, got: %v", err)
	}
	if post == nil {
		t.Fatal("nil post")
	}
...
func TestForumService_CreatePost_EmptyBody(t *testing.T) {
	svc := NewForumService(nil, nil, nil, nil, nil, nil, nil, nil)
	if _, err := svc.CreatePost(context.Background(), 1, 1, model.Permissions{}, "", nil); !errors.Is(err, ErrInvalidPost) {
		t.Errorf("expected ErrInvalidPost")
	}
}
...
func TestForumService_CreatePost_CanForumFalse(t *testing.T) {
	...
	if _, err := svc.CreatePost(context.Background(), 1, 1, model.Permissions{Level: 5}, "Reply body", nil); !errors.Is(err, ErrForumAccessDenied) {
		t.Errorf("expected ErrForumAccessDenied, got %v", err)
	}
}
...
func TestForumService_CreatePost_InvalidReplyToPostID_NotFound(t *testing.T) {
	...
	replyTo := int64(999)
	if _, err := svc.CreatePost(context.Background(), 1, 1, model.Permissions{Level: 5}, "Reply", &replyTo); !errors.Is(err, ErrInvalidReply) {
...
func TestForumService_CreatePost_ValidReplyToPostID(t *testing.T) {
	...
	replyTo := int64(50)
	if post, err := svc.CreatePost(context.Background(), 1, 1, model.Permissions{Level: 5}, "Reply", &replyTo); err != nil {
		t.Fatalf("unexpected: %v", err)
	} else if post == nil {
		t.Fatal("nil post")
	}
...
func TestForumService_GetTopic_NotFound(t *testing.T) {
	...
	if _, err := svc.GetTopic(context.Background(), 999, 1, model.Permissions{Level: 5}); !errors.Is(err, ErrTopicNotFound) {
		t.Errorf("expected ErrTopicNotFound")
	}
}
...
func TestForumService_GetTopic_ViewCountDebounce(t *testing.T) {
	...
	if topic.ViewCount != 11 {
		t.Errorf("expected 11, got %d", topic.ViewCount)
	}
	topicRepo.topicByID[1].ViewCount = 10
	topic, _ = svc.GetTopic(context.Background(), 1, 1, model.Permissions{Level: 5})
	if topic.ViewCount != 10 {
		t.Errorf("expected 10 (debounce), got %d", topic.ViewCount)
	}
	topic, _ = svc.GetTopic(context.Background(), 1, 2, model.Permissions{Level: 5})
	if topic.ViewCount != 11 {
		t.Errorf("expected 11 for diff user, got %d", topic.ViewCount)
	}
}
...
func TestForumService_Search_Success(t *testing.T) {
	...
	if got[0].PostID != 1 {
		t.Errorf("expected post ID 1")
	}
}

func TestForumService_Search_EmptyQuery(t *testing.T) {
	svc := NewForumService(nil, nil, nil, nil, nil, nil, nil, nil)
	if _, _, err := svc.Search(context.Background(), "", model.Permissions{Level: 5}, nil, 1, 25); !errors.Is(err, ErrInvalidSearch) {
		t.Errorf("expected ErrInvalidSearch, got %v", err)
	}
	if _, _, err := svc.Search(context.Background(), "   ", model.Permissions{Level: 5}, nil, 1, 25); !errors.Is(err, ErrInvalidSearch) {
		t.Errorf("expected ErrInvalidSearch for whitespace-only query, got %v", err)
	}
}

func TestForumService_Search_QueryTooShort(t *testing.T) {
	svc := NewForumService(nil, nil, nil, nil, nil, nil, nil, nil)
	if _, _, err := svc.Search(context.Background(), "a", model.Permissions{Level: 5}, nil, 1, 25); !errors.Is(err, ErrInvalidSearch) {
		t.Errorf("expected ErrInvalidSearch for single-char query, got %v", err)
	}
}
...
func TestForumService_Search_QueryTooLong(t *testing.T) {
	...
	if _, _, err := svc.Search(context.Background(), longQuery, model.Permissions{Level: 5}, nil, 1, 25); !errors.Is(err, ErrInvalidSearch) {
		t.Errorf("expected ErrInvalidSearch, got %v", err)
	}
}
