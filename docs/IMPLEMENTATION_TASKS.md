...
- **BE-10.4 — Discord + Telegram:** thin specializations of the webhook/bot-POST path. **[DONE]**
- **BE-10.5 — per-user relay: dropped** (operator decision, 2026-07-25). Per-user subscriptions and filters over the same `Announcement`/SSE machinery were always speculative; the live feeds cover the need, and each one already carries its own filters. Nothing was built for it — `Announcement.Event` stays a plain string, which was the only concession the design ever made to it.
...
##### BE-10.4: Discord + Telegram [DONE]
- **This phase is the evidence the seam held.** Two new packages, two `Register` lines in `main.go`, and one helper promoted out of the webhook package into the shared one. No migration, no new route, no change to the dispatcher, the repositories, the handlers or the schema — both kinds inherited filters, the kill-switch, dedupe, retry, dead-lettering, rate-limit coalescing, test-send and the delivery log by existing. The seam did gain one thing (below), which is the honest version of the claim.
- **Review found a real gap in the seam and it was filled: `connector.ErrPermanent`.** Alongside "not ready" there was no way for a connector to say *this will never work*. Without it a single permanently-broken Telegram chat — one that was deleted, or a group upgraded to a supergroup, which changes its id — made every healthy chat receive the same announcement five times, for every torrent, forever, because a fan-out connector fails the whole delivery when one destination fails. The drain now dead-letters such a delivery on the first attempt, so the failure lands in the admin log instead of in everyone's chat. Telegram classifies per chat (4xx permanent, 429 not-ready, 5xx/408 transient) and only reports the aggregate as permanent when every failure was.
- **Discord** posts an embed (title linking to the torrent, description from an optional template, category/size/uploader as inline fields, RFC3339 timestamp); a coalesced batch becomes plain `content` instead, because an embed titled after one torrent would misrepresent a summary standing for several. Its webhook URL **is** the credential, so it is a secret field — which is exactly why redaction strips whole config keys rather than substrings. `ValidateConfig` additionally requires https and a Discord host, so a URL pasted from the wrong place fails at save time rather than quietly shipping every announcement elsewhere.
- **Telegram** sends one `sendMessage` per chat with `parse_mode: HTML`. Every interpolated field is HTML-escaped *before* rendering, so the default template's own `<b>`/`<a href>` markup survives while a torrent name containing `&` or `<` — completely ordinary — cannot make Telegram reject the message, and no custom template can reintroduce the hole. Chat ids are strings because group ids are negative and exceed a 32-bit int, and because `@channelusername` is also valid. The bot token is refused at save time if it contains anything that could reshape a URL path.
- **The token/URL leak path got a shared fix.** `stripURL` was promoted out of the webhook package into `connector.StripURL`: for every HTTP connector the request URL is either sensitive or outright the credential (Discord's URL is the secret; Telegram's token is in the path), and `*url.Error` carries it. That error is what lands in `last_error` for the whole retention window. Redaction of configured secret values still runs on top — this is the guard for the part redaction cannot see, a URL the connector assembled rather than one stored in config.
- **FE:** Discord (write-only webhook URL, username, avatar, template) and Telegram (write-only bot token, a chat-id row editor, HTML template) sub-forms. Both kinds appeared in the kind picker without the page being told about them, because it reads `kinds` from the API.
- Tests: embed shape, coalesced content, HTML escaping (including with a custom template), per-chat fan-out, partial-failure aggregation, config validation for both, and — the ones that matter most — that neither the webhook URL nor the bot token appears in an error, before or after redaction, on both the status and the transport path. Plus a service-level test that registers the real connectors and asserts the API view and the write-only merge honour what they declare.

#### BE-10.7: Multiple live feeds, each with its own URL [M] [DONE]
**As a** member
**I want** to pick which live feed I watch — everything, or everything except a category
**So that** the live page shows me what I actually want to see, and a feed can be linked to

**Context:** operator-reported, and the natural pair to BE-10.6's exclude filters: "I want all torrents, but then one without 18+ stuff" is a filter plus a URL.

**Delivered:**
- **SSE is no longer a singleton.** Each instance carries a `slug` — lowercase letters, digits and single dashes, validated at save time because it is a URL path segment — and that slug *is* the feed's address. Migration 075 drops the singleton index, backfills `slug: "default"` onto any feed that predates it, and adds a unique index on `(config->>'slug') WHERE kind = 'sse'`. The service rejects a duplicate with a 409 first; the index is the backstop that also holds against two admins saving at the same instant.
- **`GET /api/v1/announce-stream/{slug}`**, with the unslugged route kept as an alias for the default feed so a page left open across the deploy reconnects instead of 404-ing forever. A malformed slug is a 404 — it can never name a real feed — but a *valid* slug nobody publishes to opens as a quiet stream rather than an error: refusing it would mean holding a connector list in the hub and racing every admin edit against every open connection.
- **The hub keys clients by feed.** A frame produced by one feed never reaches another's watchers, which is the entire point of having several. The per-user stream cap stays **global**: it exists to bound fan-out, and a per-feed cap would be evaded by opening each tab on a different feed.
- **`GET /api/v1/announce-feeds`** returns slug + name for enabled feeds — deliberately its own narrow view rather than the admin connector list, which carries config and filters that are behind `RequireAdmin` for a reason. A test asserts the member-facing view leaks none of it.
- **FE:** a feed picker on the live releases page, with the selection in the URL (`/live?feed=no-adult`) so a feed can be linked and the back button moves between them. Switching feeds clears both the visible list and the dedupe set — the rows on screen belong to the feed being left, and an announcement this feed has not shown must not be suppressed because the other one had. The picker is hidden unless there is more than one feed.
- **Review caught four ways a watcher could end up on the wrong feed, all fixed.** (a) The picker showed the alphabetically-first feed while a plain `/live` streamed the legacy `default` one — a member could read "Safe for work" over the unfiltered feed's releases, and could not even select the feed being displayed, because choosing an already-selected option fires no change event. The picker and the subscription now both come from one resolver. (b) Renaming a feed frees its slug; another feed taking it would silently rebind everyone still connected under the old name. Any live-feed change now disconnects watchers, who reconnect and re-resolve. (c) React runs effect cleanups after paint, so a frame from the feed just left could land in the new feed's cleared list; the list is stamped with the feed it belongs to. (d) The unique index keyed on the raw stored slug while the code read a trimmed one with a `"default"` fallback, so `" news"`/`"news"`, and any number of rows with no slug at all, coexisted as separate rows resolving to one feed. The index now keys on the effective slug, and an untrimmed slug is rejected at save time rather than trimmed on read.
- **Losing the slug race now reads as a conflict.** The service checks then inserts, so two admins saving at once can both pass; the loser hit the index and got a 500. `repository.ErrUniqueViolation` maps it back to the same 409 the check would have produced.
- **A feed's filters are not an access control** — every member may open any feed and sees every enabled feed's name. Documented in `docs/NOTIFICATION_CONNECTORS.md` and in the admin form, because a dropdown of named feeds invites an operator to create "staff-only" and assume it is one. Feed access is now a single `can_feed` privilege — see BE-10.8.
- **Rollback strips the `slug` key**, or the surviving feed would be unsaveable under the pre-075 binary, whose `ValidateConfig` rejects every key but `rate_per_min` while the admin form posts the stored config back verbatim.
- Tests: slug validation (case, spaces, slashes, `..`, leading/trailing/double dashes, padding, length), uniqueness on create and update incl. a feed keeping its own slug, per-feed fan-out isolation over a real socket (mutation-checked), the legacy alias, the cap counted across feeds, disconnect-on-change in both directions (mutation-checked), the feeds endpoint's shape/leak/500 paths, the slug index against real Postgres incl. two slugless rows and the partial-index case, and the page's picker, URL round-trip, missing-feed notice, clear-on-switch and stale-frame guard (mutation-checked).

#### BE-10.8: `can_feed` — live feed access as a privilege [S] [DONE]
**As a** tracker operator
**I want** live feed access granted by class and revocable per member
**So that** the feeds are a privilege like chat or invites, not something every account simply has

**Context:** surfaced by the BE-10.7 review, which asked for per-*feed* gating. Scoped down by operator decision (2026-07-25): access is all-or-nothing across every feed, because a member either gets the live feeds or does not, and a per-feed dimension would be a second permission model to maintain for no stated need. Being a member is still the baseline; `can_feed` is the switch on top.

**Delivered:**
- **`groups.can_feed` and `users.can_feed`** (migration 076), following exactly the shape `can_chat` and `can_invite` already use: the class grants it, and a restriction can take it away from one member without moving them out of their class. Effective access is both halves ANDed.
- **Both default to `true`.** Defaulting to false would have silently killed the live page for every member on deploy — not a decision a migration gets to make. An operator opts a class out deliberately.
- **`RestrictionTypeFeed`** joins the existing restriction machinery, so suspending feeds uses the same form, the same audit trail, the same expiry and the same restore button as every other privilege.
- **Enforced on the live rows, not the session snapshot** (`FeedAccessService`), so a revoked privilege bites on the next connect rather than whenever the member next logs in — the same reason chat re-reads the user row on every message. A lookup that fails **denies**: serving a stream to someone whose access could not be confirmed is the wrong way round for a check whose only job is to say no. An unwired gate denies too — at the handler, not just in the service, which is where review found the first cut failing open. `model.Permissions` deliberately does **not** carry `can_feed`: it would hold only the class half, so it would be wrong for any restricted member and stale the moment a class was edited — an inviting thing for a caller to branch on and always a little bit false.
- **Revoking closes what is already open**, on the `RestrictionApplied` event rather than in the admin handler. Review caught the first cut hanging it off the HTTP handler like the existing chat kick does: warning escalation applies restrictions too and never goes near that handler, so an automatically revoked member would have kept watching. Editing a class's `can_feed` disconnects every watcher (the hub keys clients by user, not group; they reconnect in seconds and are re-checked) — the gate only runs at connect, so nothing else would ever have ended those streams.
- **Both endpoints are gated**: the stream (403) and the feed list (403). Listing feeds to someone who cannot open any of them would offer a page that only ever fails.
- **Two real bugs this turned up.** `UserRepo.Create` writes the struct, so a privilege left unset at registration inserts `false` and the column default never applies — every newly registered member would have had no live feeds while every pre-existing one did. And review caught `AdminUserView` never carrying `can_feed`, which made the admin panel report "Live feeds: Suspended" for **every user on the site**, with a Restore button that changed nothing; the frontend test masked it by putting `can_feed` in its fixture, so it asserted a response shape the backend did not produce.
- **`can_feed` is a pointer on the group write request.** A client written before this existed omits the key, and a plain bool would read that as false and revoke the feeds for a whole class on an unrelated edit — which is exactly why `can_self_approve` was kept out of that request shape. Absent means "leave it as it was"; a new class gets the feeds, matching the column default rather than Go's zero value.
- **Warning escalation's "All" includes feeds**, and "Live feeds" is offered as its own restriction type — otherwise an operator locking someone down at the warning threshold would leave the feeds running.
- **FE:** "Live feeds" joins the group capability matrix and the per-user privilege list, with a "Suspend live feeds" checkbox in the restriction form. The live page reads access from the feed list rather than the stream, because `EventSource` cannot see a status code — a refused stream reaches it as an indistinguishable error, so the page would have retried every few seconds forever and read as "reconnecting" to someone who is never getting in. It now says so plainly and opens nothing.
- Tests: both halves of the AND and every combination, short-circuit on a revoked member, fail-closed on either lookup failing and on nothing being wired *at the handler*, the stream and feed-list 403s, revocation closing streams on the event (mutation-checked in both directions — a chat restriction must not close a feed), the group write request omitting/setting `can_feed`, the admin view carrying it, a **raw** insert proving the migration's column defaults (the fixture sets the field, so it never exercised one), the group repo round-tripping it through the renumbered UPDATE, registration granting it, and the page's denied state including a mid-session revoke (mutation-checked) and a 500 not being read as a refusal.

#### BE-10.6: Connector admin usability — nested category pickers, exclude filters, multiple shoutbox instances, template help [M] [DONE]
**As a** tracker operator configuring notification connectors
**I want** a category picker I can actually read, filters I can invert, several shoutbox instances, and a list of what a template may reference
**So that** setting up "announce everything except 18+ to the public feed" takes a minute and does not need the source code

**Context:** operator-reported after using the connectors page for real. The category multi-selects listed bare names in API order, so `Dub`, `Sub` and `1080p` appeared several times over with nothing to tell them apart; filters could only ever be an allow-list; chat was capped at one instance; and the template box gave no hint of what could go in it.

**Delivered:**
- **Every category picker on the site now renders the tree.** `buildCategoryOptions` was rebuilt on `buildCategoryTree`, so ordering is parent → `sort_order` → name explicitly rather than by trusting the caller's array order, and each label carries the full ancestor path (`— Movies / Dub`) because in a multi-select the parent row is often scrolled out of sight. This also fixed a bug that reached every picker on the site — upload, edit, browse, RSS: the old implementation walked roots and their direct children only, so a **third-level category was silently dropped from every category select**. The two connector selects (instance filters and IRC per-channel routing) were the ones still mapping categories raw; they now go through the same helper.
- **Category filters gained a mode: include (default) or exclude.** Exclude is what makes "everything except 18+" expressible without listing every other category. It is also why a listed category now stands for its **whole subtree**: an announcement carries only the category the torrent is in, so excluding "Adult" while a torrent sits in "Adult / 4K" would otherwise let it through — the feature would have looked like it worked. The dispatcher resolves the ancestor chain once per event onto `Announcement.CategoryPath` (`service.CategoryAncestorIDs`, cycle-guarded, shaped like the existing schema walk) and `Filters.Matches` tests against it. An unrecognised `category_mode` is rejected at parse time rather than degrading to include, which would invert the filter. Note this also changes the *include* filter for the better: listing "Movies" now matches torrents in "Movies / Action", which it previously did not.
- **Resolution failure fails closed, narrowly, and visibly.** Review caught the first cut of this being wrong in three ways. (a) `CategoryAncestorIDs` returned a *truncated chain with a nil error* for any failure past the first hop, so a connection blip would have been read as "this category has no parents" and the guard would never have fired — the exact leak it exists to prevent. Only a genuinely absent row (`sql.ErrNoRows`) is tolerated now; everything else propagates. (b) The guard withheld from *every* category-filtered instance, but an include filter falling back to the leaf can only under-match, never over-deliver — so only exclude-mode instances are held back, and include-mode ones keep their announcements. (c) A withheld instance got no delivery row at all, so nothing appeared in the log the feature already ships and there is no re-dispatch path: the announcement was simply gone. It is now written as a **failed** row carrying the reason. A nil resolver — a wiring regression in `main.go`, which is excluded from coverage — counts as "chain unknown" rather than silently downgrading every exclude filter to leaf matching.
- **IRC per-channel routing matches the subtree too.** Both selects are now fed by the same nested picker, so picking "Movies" had to mean the same thing in both; otherwise routing `#movies` to "Movies" would post nothing for a "Movies / Action" torrent while the delivery was still marked sent. `Announcement.CategoryChain()` is shared by the filter and the router, and the chain is resolved for every announcement rather than only when a filter needs it, so the stored payload's shape never depends on unrelated instances' configuration.
- **Chat is no longer a singleton** (migration 074 narrows the partial unique index to `sse` alone — 071 is untouched). Several instances post to the one shoutbox, each with its own template and filters, which is how a site words the Anime line differently from the Movies line. Overlapping filters do post twice; that is the admin's call. The Down migration restores the old index but tolerates a database that already has several chat rows rather than failing the rollback.
- **Template help comes from the backend.** `connector.TemplateFields()` derives the list from `RenderContext` by reflection and renders each example against `Sample()`, so the examples are literally what the token produces; the list ships on the connectors list response and renders as a collapsed table under each of the four template inputs. A reflection guard test fails the build if a field is added to the render context without a description — a frontend copy of the list would have drifted silently, and the failure mode is an admin writing a template against a field that no longer exists.
- **Rollback note:** `ParseFilters` uses `DisallowUnknownFields`, so an instance saved with `category_mode` and then run against an older binary fails to parse its filters *entirely* and goes silent — not just for categories. The schema rollback is tolerant (see the migration); the data is not. Clear the mode before rolling back a release.
- Tests: category option ordering / same-named children / depth ≥ 3 / orphans / placeholder-optional; a BrowsePage assertion so the shared builder cannot silently regress the four public pickers; filter include+exclude, subtree matching both ways, empty-list-in-both-modes, unknown mode rejected, leaf fallback for payloads written before `CategoryPath`; ancestor resolution incl. cycle, missing ancestor, unreadable leaf and — the one that matters — a *failed* ancestor lookup propagating rather than truncating; dispatcher path-on-payload, parent-category delivery, subtree exclusion, the withheld failed row, include-mode still delivering, resolve-once-per-event and the nil-resolver case; IRC routing a sub-category to its parent's channel and not to a sibling's; the singleton index change asserted against real Postgres in both directions; and the page's picker order, exclude round-trip and help table.

#### BE-10.7: Shoutbox announcements — deletable, readable, named, and linked [M] [BUG] [DONE]
**As a** tracker operator running the shoutbox connector
**I want** to delete a bad announcement, read the ones that stay, call the announcer something of my own, and have the torrent name be the link
**So that** the shoutbox reads like part of the site rather than a log tail nobody can edit

**Context:** operator-reported after BE-10.4/10.6 went live, from one screenshot showing all four problems at once — a system line running off the right edge of the panel, its accent bar flush against the timestamp, "System" as the author, and a bare URL trailing the torrent name.

**Delivered:**
- **Staff can delete a system message.** The backend always allowed it (`ChatService.DeleteMessage` checks only `perms.IsStaff()`, and the repo's `Delete` has no `system` filter); the block was a frontend guard, `isStaff && !msg.system`, in both `Shoutbox` and `Chat`. Announcements were therefore permanent — the bulk "delete this user's messages" path cannot reach them either, because a system row's `user_id` is NULL. The author branch is unchanged: still no profile link and no `ChatModMenu`, because there is still no user behind it; only the message is deletable.
- **Long announcements wrap.** The real cause was `min-width`, not word breaking: the message body is a flex item, flex items default to `min-width: auto`, and an unbreakable run like a URL therefore made the row wider than the panel instead of wrapping. `min-width: 0` plus `overflow-wrap: anywhere` replaces the non-standard `word-break: break-word`.
- **The system row is legible.** Padding, a 2px *transparent* left border and the radius moved onto the **base** `.__message` rule, so the system variant only changes colour. Putting them on the variant alone was what pushed the timestamp column sideways on announcements and left the accent bar touching the time. Tint deepened from 92% to 88% transparent. Both `shoutbox.css` and `chat.css` — they are duplicated, not shared.
- **`chat_system_display_name` is a site setting** (migration 077, seeded `System` so the migration changes nothing on its own; the row has to exist because the Site Settings page renders the rows the API returns, not the frontend's definition list). Validation rejects blank and counts **runes**, not bytes, against the 32-character cap — a byte check would reject a CJK label at a third of the limit.
- **One owner for the label.** It was hardcoded in four places; the SQL `CASE WHEN cm.system THEN 'System'` was the dangerous one, a copy no setting change could ever reach. The repository now returns system rows unlabelled and `ChatService` stamps them on the way out (`SendSystemMessage`, `ListRecent`, `ListHistory` — the only read paths), resolved lazily so a page with no announcements pays for no settings read. The chat connector's hand-built WebSocket payload uses the returned `msg.Username` rather than the constant, so the live line and the same line after a reload cannot disagree.
- **Cached, because it is read per announcement and changes ~never.** `SiteSettingsService.SystemChatName` is the *only* cached read on the service, deliberately: a general-purpose cached getter would invite the same treatment for `connectors_enabled`, where an operator flipping the kill-switch expects the site to obey now. `Set` evicts immediately (exact in-process) and a 5-minute TTL bounds staleness on a replica that did not serve the write. `sql.ErrNoRows` is cached as the fallback — it is a stable answer — while any other error is not, so a connection blip cannot pin the default for the whole TTL.
- **`{{.Link}}` renders the torrent name as a Markdown link**, escaped on both sides. It is a built-in rather than advice to write `[{{.Name}}]({{.URL}})`, because scene names contain brackets constantly and `[SubsPlease] Show` would close the label early and spill the rest as literal text. The shoutbox connector gets its own `DefaultTemplate` using it, substituted in `Deliver` before `RenderTemplate` (whose empty-template fallback is the shared plain-text default); `connector.DefaultTemplate` is untouched because it also feeds IRC and `Announcement.Body`. The reflection guard test forces the new field to be documented, so the help table on the Connectors page picked it up for free.
- **Migration 078 relinks instances created before this.** Only rows whose template is *exactly* the old shared default are rewritten — a hand-written template is the admin's wording. Rows with an empty template are deliberately left empty: they already track whatever the code default is, and pinning one would take that away. IRC rows holding the same string are untouched.
- **Escaping is a security boundary, not just cosmetics.** Torrent names are uploader-controlled and stored verbatim, and the shoutbox renders Markdown — so `<`, `>`, `&` and `!` are escaped alongside the link and emphasis characters. Left raw, a name like `Ping <https://evil.example> now` renders as a *second*, attacker-chosen link inside a row that reads as an official site announcement (not XSS — `rehypeSanitize` and inline mode handle that — but a phishing link every logged-in member sees). The label is also whitespace-collapsed, because a newline reads as a paragraph break and splits the link in half.
- **The label is length-bounded (200 bytes, measured on the escaped output).** The shoutbox refuses a message over 500 **bytes** and a name is up to 255 characters, so an adversarially punctuated name — escaping costs a byte per escaped character — could otherwise render an announcement its own destination rejects, which burns the retries and dead-letters. Counted in bytes rather than runes deliberately: multi-byte runes are not escapable, so a rune budget would have permitted three times the intended size.
- **Rollback note:** migration 078 writes `{{.Link}}` into stored templates, and the old `RenderContext` has no such field — so rolling the *binary* back without running 078's Down makes `RenderTemplate` error and dead-letters every shoutbox delivery for those instances, and re-saving the instance fails `ValidateConfig` too. Run the Down, or clear the template, before rolling back a release. (Same shape as BE-10.6's `category_mode` note.)
- **Known limits, deliberately not addressed here** (see the two follow-up stories below): deleting a chat message still writes no activity-log entry, and a `!!spoiler!!` in a torrent name still expands into a block-level disclosure widget inside the row. Both predate this change; neither is caused by it.
- Tests: Markdown escaping table (brackets, parens, emphasis markers, a backslash escaped once not twice, parens in the destination, and empty-URL yielding the bare label); `{{.Link}}` through the renderer; a guard that the shared default stays plain text; the chat connector's default asserted as an exact string *and* asserted not to trail a bare URL; the broadcast payload's username via a fake returning a name that is **not** `model.SystemChatUsername`, so a hardcoded constant cannot pass; settings cache (read-once, missing-row cached, storage failure *not* cached, `Set` evicting, TTL expiry via an injected clock) and validation (blank, over-long, runes-not-bytes, trimmed); service labelling on send/list/history with authored rows untouched, the nil-settings fallback, and no settings read for a page without announcements; Postgres — system rows come back unlabelled, 077 seeds the row, and 078 asserted by rolling **back** to 077 over seeded rows and forward again (stock rewritten, custom untouched, bare left bare, IRC untouched); frontend — the delete button present for staff and reaching the admin endpoint (mutation-checked in both components), hidden from non-staff, the label taken from the payload rather than a hardcoded "System", and the connector's escaped Markdown rendering as a real `<a>` with the brackets in its text.

#### BE-10.8: Audit-log chat message deletions [S] [BUG]
**As a** tracker operator
**I want** chat and shoutbox deletions recorded in the activity log
**So that** a removed message — including a site announcement — leaves a trace of who removed it

**Context:** review finding from BE-10.7. `ChatService.DeleteMessage` publishes `ChatMessageDeletedEvent` (and `DeleteUserMessages` publishes `ChatUserMessagesDeletedEvent`), but **no listener subscribes to either** — `internal/listener/activity_log.go` covers `ChatUserMuted`/`ChatUserUnmuted` only. The gap predates BE-10.7; what changed is that system announcements are now deletable too, so a site-wide notice can be removed with no record at all.

**Acceptance criteria:**
- `RegisterActivityLogListeners` handles both events.
- The actor's username is resolved: `event.NewBase(event.ChatMessageDeleted, event.Actor{ID: actorID})` carries no `Username`, so the entry would otherwise read "` deleted message 9`". The listener already receives a `userRepo`.
- Decide the visibility: chat moderation is currently *public* in the log (BE-9.24 left it out of the staff-only set). Confirm that is still right for message deletion, or add both types to `event.StaffOnlyTypeStrings`.
- Tests: both events produce an entry, the actor name is resolved, and the classification is asserted either way.

#### FE-4.6: A spoiler inside a link must not split the row [S] [BUG]
**As a** member reading the shoutbox
**I want** an announcement to stay on one line whatever the torrent is called
**So that** a release named with `!!` does not turn a chat row into a disclosure widget

**Context:** review finding from BE-10.7. `remarkSpoiler` rewrites `!!…!!` anywhere in the tree, including inside a `link` node, so a torrent named `Show !!secret ending!! 01` renders as `<a>Show </a><details><summary>Spoiler</summary><a>secret ending</a></details><a> 01</a>` — a block-level widget inside a one-line row, with the link split three ways. Backslash-escaping does not help: the plugin runs on the mdast after escapes have resolved, which is why BE-10.7's escaping could not close it. Pre-existing — the old template interpolated the raw name into the same Markdown-rendered message.

**Acceptance criteria:**
- `remarkSpoiler` skips descendants of a `link` node (a spoiler whose reveal is a link label is not a meaningful construct anyway).
- Tests: `[Show !!x!! 01](url)` renders as a single `<a>` with literal `!!`, and a spoiler outside a link still works — in `MarkdownRenderer.test.tsx` and in the shoutbox row.
